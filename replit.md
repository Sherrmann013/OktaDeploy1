...
- **SAVE BUTTON PERSISTENCE FIX (January 2025):** Resolved critical issue where individual save buttons appeared to save but changes didn't persist after page refresh. Root cause: useEffect hooks processing server data were automatically clearing unsaved changes flags (setHasDepartmentUnsavedChanges(false)) when data loaded, causing save functions to think no changes existed. Solution: Removed automatic flag clearing from all data processing useEffects for department/employee type app and group mappings. Individual save buttons now properly detect and persist changes with complete data integrity.
- **INDIVIDUAL SAVE BUTTONS IMPLEMENTED (January 2025):** Discovered and fixed critical UI gap where individual save buttons for app/group mappings didn't exist, causing user confusion. Root cause: Auto-triggering direct save function was being called automatically instead of by user interaction, and there were no actual individual save buttons in the interface. Solution: (1) Disabled auto-triggering useEffect that was calling save functions automatically, (2) Added actual individual save buttons to UI that appear when unsaved changes exist, (3) Connected buttons directly to specific save functions (saveDepartmentAppMappings, saveEmployeeTypeAppMappings, etc.) with proper manual save flags. Users now have proper individual save buttons for department/employee type app and group mappings.
- **UNIFIED SAVE SYSTEM (January 2025):** Consolidated save functionality into single main save button per user request, eliminating need for multiple save buttons. Modified saveCurrentFieldChanges function to handle both field configuration and app/group mapping assignments in one action. Removed individual save buttons from interface for cleaner, more intuitive user experience. Main save button now handles both layout_settings table (field config) and mapping tables (app/group assignments) simultaneously.
- **CLIENT DATABASE SCHEMA FIX (January 2025):** Resolved critical database column name mismatch preventing email group mapping persistence. Root cause: Client database initialization created tables with `okta_group_name` columns while server endpoints expected `group_name` columns (matching shared schema). Solution: Dropped incorrect client database tables and recreated with proper schema matching server-side expectations. Fixed both department and employee type group mapping endpoints. All app and email group mappings now persist correctly with full CRUD functionality.
