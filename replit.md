...
- **Unified Integration Management IMPLEMENTED:** Dashboard layout "Add Integration" now pulls from the same database integrations as the main integrations tab, ensuring single source of truth. Added "Custom Integration" option exclusively to dashboard layout (not main integrations) for flexibility while maintaining consistency. Custom cards get purple plus icon and are stored with type "custom" for proper visual distinction.
